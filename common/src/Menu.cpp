...
Menu::Menu(const std::string& menuTitle, const std::string& icon)
    : currentSelection(0), title(menuTitle), titleIcon(icon) {
...
void Menu::addOption(const std::string& option) {
    options.push_back(MenuOption(option, ""));
}

void Menu::addOption(const std::string& option, const std::string& icon) {
    options.push_back(MenuOption(option, icon));
}
...
void Menu::drawTitle() {
    ...
    // Draw title centered with optional icon
    std::string fullTitle = titleIcon.empty() ? title : titleIcon + " " + title;
    int titleX = startX + (boxWidth - static_cast<int>(fullTitle.length())) / 2;
    attron(A_BOLD | A_UNDERLINE);
    mvprintw(startY + 2, titleX, "%s", fullTitle.c_str());
...
void Menu::drawOptions() {
    ...
    for (size_t i = 0; i < options.size(); i++) {
        ...
        // Build option string with icon if available
        std::string optionText = options[i].icon.empty()
            ? options[i].text
            : options[i].icon + "  " + options[i].text;

        if (static_cast<int>(i) == currentSelection) {
            attron(A_REVERSE | A_BOLD);
            mvprintw(optionY, optionX, "> %s", optionText.c_str());
