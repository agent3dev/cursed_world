...
void PopulationManager::evolveCats(TerminalMatrix& matrix) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(parentsCount) && i < cats.size(); i++) {
        parents.push_back(std::move(cats[i]));
    }

    // Place skulls for removed cats (bottom 50%)
    for (size_t i = static_cast<size_t>(parentsCount); i < cats.size(); i++) {
        if (cats[i]) {
    ...
    // Pre-extract parent weights to avoid repeated getWeights() calls
    std::vector<std::vector<double>> parentWeightCache;
    parentWeightCache.reserve(cats.size());
    for (const auto& cat : cats) {
        parentWeightCache.push_back(cat->getBrain().getWeights());
    }
...
void PopulationManager::evolveGeneration(TerminalMatrix& matrix) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(parentsCount) && i < population.size(); i++) {
    ...
    // Pre-extract parent weights to avoid repeated getWeights() calls
    std::vector<std::vector<double>> parentWeightCache;
    parentWeightCache.reserve(population.size());
    for (const auto& rodent : population) {
        parentWeightCache.push_back(rodent->getBrain().getWeights());
    }
